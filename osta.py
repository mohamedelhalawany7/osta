...
def get_import_error():
    """تجربة استيراد main.py وإرجاع الخطأ الحقيقي"""
    try:
        ...
        try:
            ...
            if spec is None:
                return "main.py غير موجود في المسار: " + os.getcwd()
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            return None  # لا يوجد خطأ
        except Exception:
...
def start_fastapi():
    ...
    # فحص وجود main.py أولاً
    main_path = os.path.join(os.getcwd(), "main.py")
    if not os.path.exists(main_path):
        files = os.listdir(os.getcwd())
        return False, f"main.py غير موجود!\nالملفات الموجودة: {files}"

    # فحص الاستيراد
    err = get_import_error()
    if err:
        return False, f"خطأ في استيراد main.py:\n{err}"
    ...
    def run():
        try:
            ...
            import traceback
            error_container["msg"] = traceback.format_exc()
    ...
    return False, error_container["msg"] or "انتهى وقت الانتظار - السيرفر لم يبدأ"
...
def start_ngrok():
    ...
    try:
        ...
        import traceback
        st.session_state["ngrok_error"] = traceback.format_exc()
        return None
...
def show_loading(msg):
    ...
    <div style="display:flex;align-items:center;justify-content:center;
        height:60vh;background:#0A0E17;color:#00F0FF;
...
def main():
    ...
    if "fastapi_started" not in st.session_state:
        ...
        ok, err = start_fastapi()

        if not ok:
            st.error("❌ فشل تشغيل FastAPI")
            st.code(err, language="text")

            # معلومات تشخيصية
            st.markdown("### 🔍 معلومات تشخيصية:")
            st.write(f"**مسار العمل الحالي:** `{os.getcwd()}`")
            try:
                files = os.listdir(os.getcwd())
                st.write(f"**الملفات الموجودة:** `{files}`")
            except Exception as e:
                st.write(f"خطأ في قراءة الملفات: {e}")
    ...
    if "ngrok_url" not in st.session_state:
        ...
        if not url:
            err = st.session_state.get("ngrok_error", "خطأ غير معروف")
            st.warning("⚠️ تعذر فتح ngrok")
            st.code(err, language="text")
            st.info("أضف NGROK_TOKEN في Streamlit Secrets من ngrok.com مجاناً")
            st.markdown(f"""
            <div style="background:#0A0E17;border:1px solid #00F0FF;
                ...
                <h2 style="color:#fff;margin:16px 0;">السيرفر يعمل محلياً فقط</h2>
