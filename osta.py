...
# =====================================================================
# التشفير والاتصال بقاعدة بيانات Firebase (التعديلات الجراحية 3 و 4)
# =====================================================================
...
# الحل الجراحي 4: معالجة خطأ Invalid salt
...
def init_firebase():
    if not firebase_admin._apps:
        try:
            # الحل الجراحي 3: قراءة ملف الـ JSON كـ String من Secrets
...
def chat_view():
    ...
    # الحل الجراحي 1: إلغاء order_by لتجنب Composite Index
    # جلب الجلسات عبر where فقط، ثم ترتيبها برمجياً (In-Memory Sort)
    ...
    with st.sidebar:
        ...
        for s_doc in raw_sessions:
            ...
            # الحل الجراحي 2: معالجة session_uuid
            ...
            if st.button(f"💬 {title[:20]}", key=f"sel_{s_id}", type=btn_type, use_container_width=True):
                ...
                # جلب الرسائل الخاصة بالجلسة المحددة (بدون order_by لتجنب الفايربيز إندكس)
    ...
    if prompt := st.chat_input("سجل عطل المكنة هنا..."):
        ...
        if not st.session_state.get('active_chat_id'):
            ...
            # تحديث session_uuid داخل الدوكيومنت نفسه (لتوافقية الحل #2)
...
def admin_dashboard():
    ...
    # الحل الجراحي 1 (In-Memory Sort للمنبهات)
    ...
    if alerts_docs:
        for a_doc in alerts_docs:
            a = a_doc.to_dict()
            st.warning(f"**عطل:** {a.get('details', '')} | **التاريخ:** {a.get('created_at', '')}")
    else:
