...
def init_firebase():
    ...
    try:
        # استخراج project_id والمفاتيح بطريقة آمنة لتجهيزها
        if "FIREBASE_KEY" in st.secrets:
            fb_config = json.loads(st.secrets["FIREBASE_KEY"])
        ...
        st.error(f"خطأ في قراءة مفاتيح Firebase: {e}")
        return None

    # فرض وضع project_id في بيئة التشغيل لضمان رؤية firestore.client() له في كل مرة
    if project_id:
        os.environ["GOOGLE_CLOUD_PROJECT"] = project_id

    if not firebase_admin._apps:
        try:
            if cred and project_id:
                # تمرير project_id صراحة إلى التهيئة لمنع أي فقدان للبيانات في السحابة
